...
const select_having = (having, component, children, nest) => {
    ...
    invariant(
        type === EXPR_TYPE.BINARY_EXPR || validValueTypes.includes(type),
        `Unsupported HAVING expression type: ${type}`
    );

    if (type !== EXPR_TYPE.BINARY_EXPR && type !== EXPR_TYPE.UNARY_EXPR) {
        clause_value(having, component, children, nest);
    } else {
        // certainly an expression with an operator

        // ensure alphabetic operators are always upper-case by convention
        having.operator = having.operator.toUpperCase();

        const { operator } = having;

        invariant(
            SUPPORTED_OPERATORS.SELECT.HAVING.some(
                (supportedOp) => supportedOp === operator
            ),
            `Unsupported HAVING expression operator: ${operator}`
        );

        if (type === EXPR_TYPE.BINARY_EXPR) {
            const { left, right } = having;
            if (cmpOperators[operator]) {
                where_cmp(operator, left, right, component, children, nest);
            } else if (logicalOperators[operator]) {
                where_log(operator, left, right, component, children, nest);
                ...
                console.error(`unhandled operator for SELECT ... HAVING: ${operator}`);
            }
        } else if (type === EXPR_TYPE.UNARY_EXPR) {
            // NOT is the only unary operator in SUPPORTED_OPERATORS, so operator === 'NOT' is surely true
            unary_op(operator, having, component, children, nest);
        }
    }
};

const select_groupby = (group_by, component, children, nest) => {
    children.push(generateSpanChild("GROUP BY"));

    const { type } = group_by;

    clause_value(group_by, component, children, nest);
};

const select_orderby = (order_by, component, children, nest) => {
    children.push(generateSpanChild("ORDER BY"));

    const { type } = order_by;

    clause_value(order_by, component, children, nest);
...
const clauseHandlers = {
  select: {
    ...
    order_by: select_orderby,
    group_by: select_groupby,
  },
};
